...
struct llama_mmap::impl {
    ...
    impl(struct llama_file * file, size_t prefetch, bool numa, void * model_addr, size_t model_size) {
        if (model_addr != nullptr && model_size != 0) {
            // Decrypted model blob in memory: treat it as the backing store.
            addr = model_addr;
            size = model_size;
            mapped_fragments.emplace_back(0, model_size);
            GGML_UNUSED(file);
...
llama_mmap::llama_mmap(struct llama_file * file, size_t prefetch, bool numa, void * model_addr, size_t model_size) :
    pimpl(std::make_unique<impl>(file, prefetch, numa, model_addr, model_size)) {}
