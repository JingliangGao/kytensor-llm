...
static void llama_adapter_lora_init_impl(
    ...
    if (decrypt_type != GGUF_DECRYPT_TYPE_NONE)
        model.decrypt_lora_list.emplace_back(gguf_get_model_addr(ctx_gguf.get()), std::free);
    ...
    // set tensor data
    {
        // If the LoRA file was decrypted by gguf_init_from_file(), read it from decrypted memory.
        void * lora_model_addr = gguf_get_model_addr(ctx_gguf.get());
        size_t lora_model_size = gguf_get_model_size(ctx_gguf.get());

        FILE * lora_fp = nullptr;
        llama_file * gguf_file = nullptr;

#if !defined(_WIN32)
        if (lora_model_addr != nullptr && lora_model_size > 0) {
            lora_fp = fmemopen(lora_model_addr, lora_model_size, "rb");
            if (!lora_fp) {
                throw std::runtime_error(format("%s: fmemopen failed for '%s': %s", __func__, path_lora, strerror(errno)));
            }
            gguf_file = new llama_file(lora_fp);
        }
#endif

        if (gguf_file == nullptr) {
            gguf_file = new llama_file(path_lora, "rb");
        }
        ...
        delete gguf_file;
        if (lora_fp) {
            fclose(lora_fp); // gguf_file(FILE*) does not own fp
        }
    }
...
llama_adapter_lora * llama_adapter_lora_init(
    ...
    try {
        llama_adapter_lora_init_impl(*model, path_lora, *adapter, decrypt_type);
        return adapter;
