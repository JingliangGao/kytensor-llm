...
public:
    ...
    // expand the cell arrays to n cells, preserving existing cell metadata.
    // only the new cells (from old size to n) are initialized as empty.
    // used and seq_pos maps are NOT cleared.
    void resize_expand(uint32_t n) {
        uint32_t old_n = pos.size();
        if (n <= old_n) return;
        pos.resize(n);
        ext.resize(n);
        shift.resize(n);
        seq.resize(n);
        for (uint32_t i = old_n; i < n; ++i) {
            pos[i]   = -1;
            ext[i].reset();
            shift[i] =  0;
            seq[i].reset();
