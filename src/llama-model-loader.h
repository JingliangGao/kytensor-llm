...
// Thrown when gguf decryption or tensor loading is cancelled by the user.
struct llama_model_load_cancelled : std::exception {
    const char * what() const noexcept override {
        return "model load cancelled";
    }
};
...
struct llama_model_loader {
    ...
    llama_files files;
    // If the corresponding GGUF was decrypted, these will point to the decrypted blob in memory.
    // Indices match `files` and `llama_tensor_weight::idx`.
    std::vector<void *> model_addrs;
    ...
    // TODO: add by guoxing.xu to store the name of the model
    std::string fname_;
