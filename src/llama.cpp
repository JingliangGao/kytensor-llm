...
static int llama_model_load(struct gguf_context * metadata, llama_model_set_tensor_data_t set_tensor_data, void * set_tensor_data_ud,
    ...
    try {
        llama_decrypt_abort_callback decrypt_abort_callback = params.decrypt_abort_callback;
        void * decrypt_abort_user_data = params.decrypt_abort_user_data;

        llama_model_loader ml(metadata, set_tensor_data, set_tensor_data_ud, fname, splits, file, params.use_mmap, params.use_direct_io,
            params.check_tensors, params.no_alloc, params.kv_overrides, params.tensor_buft_overrides, params.decrypt_type,
            decrypt_abort_callback, decrypt_abort_user_data);
    ...
    } catch (const llama_model_load_cancelled &) {
        return -2;
        ...
        if (params.split_mode == LLAMA_SPLIT_MODE_TENSOR) {
            ...
            for (size_t i = 0; i < ggml_backend_dev_count(); ++i) {
                ...
                switch (ggml_backend_dev_type(dev)) {
                    ...
                    case GGML_BACKEND_DEVICE_TYPE_HM_ACCEL:
