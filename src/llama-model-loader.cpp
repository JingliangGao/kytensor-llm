...
llama_model_loader::llama_model_loader(
	...
	const enum gguf_decrypt_type decrypt_type,
        llama_decrypt_abort_callback decrypt_abort_callback,
        void *                       decrypt_abort_user_data)
    ...
    // Encrypted GGUF files must be decrypted before parsing/mapping.
    // However, `no_alloc=true` (used during `--fit`) must not force the mmap buffer-from-host-ptr path,
    // because that path asserts `!ml.no_alloc`.
    if (decrypt_type != GGUF_DECRYPT_TYPE_NONE && !no_alloc) {
        use_mmap = true;
        use_direct_io = false;
    }
    ...
    if (!fname.empty()) {
        ...
        struct gguf_init_params params = {
            ...
            /*.decrypt_abort_callback = */ decrypt_abort_callback,
            /*.decrypt_abort_user_data = */ decrypt_abort_user_data,
        };

        metadata_ptr.reset(gguf_init_from_file(fname.c_str(), params));
        metadata = metadata_ptr.get();
        if (metadata == nullptr) {
            if (gguf_get_last_init_error() == GGUF_INIT_ERR_ABORTED) {
                throw llama_model_load_cancelled();
            }
        ...
        // Keep decrypted blob info aligned with `files` order (file index == 0).
        model_addrs.emplace_back(gguf_get_model_addr(metadata));
        model_sizes.emplace_back(gguf_get_model_size(metadata));
        ...
        if (n_split > 1) {
            ...
            for (idx = 1; idx < n_split; idx++) {
                ...
                contexts.emplace_back(ctx);
                model_addrs.emplace_back(gguf_get_model_addr(ctx_gguf.get()));
                model_sizes.emplace_back(gguf_get_model_size(ctx_gguf.get()));
        ...
        struct gguf_init_params params = {
            /*.no_alloc = */ true,
            /*.ctx      = */ &ctx,
            /*.decrypt_type = */ GGUF_DECRYPT_TYPE_NONE,
        };
    ...
    if (decrypt_abort_callback != nullptr && decrypt_abort_callback(decrypt_abort_user_data) == 1) {
        throw std::runtime_error(format("%s: %d, aborted to load GGUF\n", __func__, __LINE__));
    }
...
void llama_model_loader::init_mappings(bool prefetch, llama_mlocks * mlock_mmaps) {
    if (use_mmap) {
        ...
        for (size_t idx = 0; idx < files.size(); idx++) {
            ...
            size_t model_size = 0;
            if (idx < model_addrs.size()) {
                model_addr = model_addrs.at(idx);
                model_size = model_sizes.at(idx);
            }
            bool is_numa = false;
            ...
            std::unique_ptr<llama_mmap> mapping = std::make_unique<llama_mmap>(file.get(), prefetch ? -1 : 0, is_numa, model_addr, model_size);
...
void llama_model_loader::load_data_for(struct ggml_tensor * cur) const {
    ...
    if (use_mmap) {
        ...
        if (w.idx < model_addrs.size() && model_addrs.at(w.idx) != nullptr && model_sizes.at(w.idx) > 0) {
            memcpy(cur->data, (uint8_t *) model_addrs.at(w.idx) + w.offs, ggml_nbytes(cur));
        } else {
