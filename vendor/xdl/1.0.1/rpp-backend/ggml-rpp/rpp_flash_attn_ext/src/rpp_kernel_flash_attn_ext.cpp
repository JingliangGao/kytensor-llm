...
static bool ggml_rpp_create_engine_flash_attention(ggml_backend_rpp_context & ctx,
    ...
    const bool has_mask = dst->src[3] != nullptr;
    if (has_mask && dst->src[3]->data == nullptr) {
        GGML_LOG_ERROR("%s: FLASH_ATTN_EXT mask tensor has no device data\n", __func__);
        return false;
    }
    ...
    void * i_buffers[4] = {
        ...
        dst->src[1]->data,
        dst->src[2]->data,
        has_mask ? dst->src[3]->data : nullptr,
    };
    ...
    int mask_bytes_per_elem =
        has_mask ? ggml_rpp_get_io_type_size(ctx, dst->src[3]) : (int) sizeof(rpp::bfloat16);

    rpp_flash_atten_build(*(rpp_node->kernel_ctx.get()), Tq, Tk, Hq, Hkv, D, scale, kv_bytes_per_elem,
                          io_bytes_per_elem, mask_bytes_per_elem, has_mask, rpp_node->is_instantial);
...
static bool ggml_rpp_flash_attn_ext_properties_is_same(ggml_backend_rpp_context & ctx,
    ...
    const bool has_mask           = node->src[3] != nullptr;
    const bool saved_has_mask     = graph_node_properties.src_address[3] != nullptr;
    if (has_mask != saved_has_mask) {
        return false;
    }
