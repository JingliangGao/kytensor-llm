...
static bool ggml_rpp_create_kernel_get_rows(ggml_backend_rpp_context & ctx,
    ...
    // Use warmup to get elements_per_row
    GGML_ASSERT(ggml_n_dims(dst->src[0]) == 2);
    const int elements_per_row = dst->src[0]->ne[0];
    rpp_get_rows_build(*(rpp_node->kernel_ctx.get()), elements_per_row, i_type_size, o_type_size, ggml_type_size(dst->src[1]->type),rpp_node->is_instantial);

    // Build an MPU source-address update followed by one D2D copy per requested row.
    // rpp_get_rows_build(*(rpp_node->kernel_ctx.get()), dst, i_type_size, o_type_size, rpp_node->is_instantial);
    return true;
}
...
static bool ggml_rpp_get_rows_properties_is_same(ggml_backend_rpp_context & ctx,
    ...
    // The MPU graph has one update/copy pair per output row, so its shape is fixed.
    // for (int i = 0; i < GGML_MAX_DIMS; i++) {
    //     if (node->ne[i] != graph_node_properties.ne[i] || node->nb[i] != graph_node_properties.nb[i]) {
    //         return false;
    //     }
    // }
