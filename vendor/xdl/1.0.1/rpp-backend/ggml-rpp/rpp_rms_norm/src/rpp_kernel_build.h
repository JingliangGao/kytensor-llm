...
static RPPdeviceptr rmsnorm_prepare_rsqrt_lut_workspace(rpp_kernel_context & ctx, int lut_elements) {
    const int lut_bytes = lut_elements * (int) sizeof(uint16_t);
    rpp_dev_lut_key key{};
    key.kind  = rpp_dev_lut_kind::rms_norm_rsqrt_lut;
        ...
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [=](RPPdeviceptr workspace, size_t bytes) {
                ...
                for (uint32_t i = 0; i < (uint32_t) lut_elements; i++) {
                    ...
                    float    v;
                    memcpy(&v, &x, sizeof(v));
                    const float y = 1.0f / std::sqrt(v);
                    rsqrt_table[i] = rpp::bfloat16::round_to_bfloat16(y).value;
                }
                rsqrt_table[0]      = 0;
                rsqrt_table[0x8000] = 0;
                if (rtMemcpy((void *) workspace, rsqrt_table.data(), bytes, rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize RMSNorm rsqrt LUT");
                }
            });
...
static void rpp_rmsnorm_common_build_legacy(rpp_kernel_context & ctx,
    ...
    RPPdeviceptr dev_rsqrt_lut = rmsnorm_prepare_rsqrt_lut_workspace(ctx, lut_elements);
