...
inline RPPdeviceptr l2norm_prepare_rsqrt_lut_workspace(rpp_kernel_context & ctx) {
    ...
    rpp_dev_lut_key key{};
    key.kind  = rpp_dev_lut_kind::l2_norm_rsqrt_lut;
        ...
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [](RPPdeviceptr workspace, size_t bytes) {
                ...
                for (uint32_t i = 0; i < (uint32_t) lut_elements; i++) {
                    ...
                    rsqrt_table[i] = rpp::bfloat16::round_to_bfloat16(1.0f / std::sqrt(value)).value;
                }
                rsqrt_table[0]      = 0;
                rsqrt_table[0x8000] = 0;
                if (rtMemcpy((void *) workspace, rsqrt_table.data(), bytes, rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize L2Norm rsqrt LUT");
                }
            });
