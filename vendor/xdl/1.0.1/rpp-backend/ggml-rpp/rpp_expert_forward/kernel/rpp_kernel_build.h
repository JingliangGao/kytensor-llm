...
inline RPPdeviceptr prepare_quant_lut_workspace(int quant, int device) {
    rpp_kernel_context lut_ctx{};
    lut_ctx.device = device;
    switch (quant) {
        case 0:
...
inline void copy_quant_lut_to_sram(rpp_kernel_context &          ctx,
                                   ...
                                   int                           quant,
                                   RPPstream &                   stream) {
    const RPPdeviceptr dev_lut_workspace = prepare_quant_lut_workspace(quant, ctx.device);
