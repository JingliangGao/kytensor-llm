...
static inline RPPdeviceptr q2s_vxm_nolut_prepare_lut_workspace(rpp_kernel_context & ctx) {
    ...
    key.variant = (uint32_t) GGML_TYPE_IQ2_S | RPP_DEV_LUT_VARIANT_NOLUT;
    key.bytes   = q2s_vxm_nolut_lut_workspace::total_bytes;
    ...
    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [](RPPdeviceptr workspace, size_t) {
                ...
                constexpr std::array<float, q2s_vxm_nolut_lut_workspace::mag_lut_elems> values = {
                ...
                if (rtMemcpy((void *) workspace, qscale_lut.data(),
                             q2s_vxm_nolut_lut_workspace::qscale_lut_bytes, rtMemcpyHostToDevice) != rtSuccess ||
                    rtMemcpy((void *) (workspace + q2s_vxm_nolut_lut_workspace::qscale_lut_bytes), mag_lut.data(),
                             q2s_vxm_nolut_lut_workspace::mag_lut_bytes, rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize Q2S VXM NoLUT LUT");
                }
            });
