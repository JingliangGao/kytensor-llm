    ...
    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [](RPPdeviceptr workspace, size_t) {
                ...
                const std::array<float, 4> values = { 8.0f, 25.0f, 43.0f, 0.0f };
                ...
                if (rtMemcpy((void *) workspace, qscale_lut.data(), qscale_lut_bytes,
                    ...
                    rtMemcpy((void *) (workspace + qscale_lut_bytes), mag_lut.data(), mag_lut_bytes,
                             rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize Q2XS VXM NoLUT LUT");
                }
            });
