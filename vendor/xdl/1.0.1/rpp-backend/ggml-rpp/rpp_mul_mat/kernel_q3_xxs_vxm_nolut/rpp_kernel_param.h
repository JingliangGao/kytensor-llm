...
static RPPdeviceptr q3xxs_vxm_nolut_prepare_lut_workspace(rpp_kernel_context & ctx) {
    ...
    key.variant = (uint32_t) GGML_TYPE_IQ3_XXS |
                  RPP_DEV_LUT_VARIANT_NOLUT | RPP_DEV_LUT_VARIANT_VXM_LAYOUT;
    key.bytes   = total_bytes;
    ...
    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [=](RPPdeviceptr workspace, size_t) {
                ...
                if (rtMemcpy((void *) (workspace + off_qscale), qscale_lut.data(),
                             q3xxs_vxm_nolut_lut_workspace::qscale_lut_bytes,
                             rtMemcpyHostToDevice) != rtSuccess ||
                    rtMemcpy((void *) (workspace + off_mag), mag_lut.data(),
                             q3xxs_vxm_nolut_lut_workspace::mag_lut_bytes,
                             rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize Q3XXS VXM NoLUT LUT");
                }
            });
