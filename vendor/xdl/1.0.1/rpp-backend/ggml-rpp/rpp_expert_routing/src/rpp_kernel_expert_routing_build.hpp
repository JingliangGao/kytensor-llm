...
RPPdeviceptr inline rpp_expert_routing_prepare_lut_workspace(rpp_kernel_context & ctx, uint64_t lut_table_size) {
    ...
    rpp_dev_lut_key key{};
    key.kind  = rpp_dev_lut_kind::expert_routing_lut;
    ...
    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [=](RPPdeviceptr workspace, size_t) {
                ...
                if (rtMemcpy((void *) workspace, exp_table.data(), lut_table_size, rtMemcpyHostToDevice) !=
                        rtSuccess ||
                    rtMemcpy((void *) (workspace + lut_table_size), reciprocal_table.data(), lut_table_size,
                             rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize ExpertRouting LUT");
                }
            });
