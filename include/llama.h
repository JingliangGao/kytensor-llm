...
extern "C" {
    ...
    // Called during Tongyi GGUF decryption before each encrypted file is processed.
    // Return 0 to continue, non-zero to abort (same contract as tongyi_decrypt_abort_callback).
    // Set abort_user_data to volatile int * and use llama_decrypt_abort_flag as the callback
    // to abort from another thread by writing *flag = 1.
    typedef int (*llama_decrypt_abort_callback)(void * user_data);
    ...
    struct llama_model_params {
        ...
        // Optional model decrypt type for GGUF encrypted models.
        enum gguf_decrypt_type decrypt_type;

        // optional: abort Tongyi model decryption (NULL = no abort checks)
    ...
    // Returns the number of discarded tokens, or 0 on failure.
    LLAMA_API int32_t llama_memory_shift(
            llama_memory_t mem,
              llama_seq_id seq_id,
                 llama_pos n_past,
                 int32_t   n_keep);

    // Context extension via Self-Extend (Grouped Attention)
    LLAMA_API void llama_memory_shift_extend(
            llama_memory_t mem,
              llama_seq_id seq_id,
                llama_pos * n_past,
                int32_t   * ga_i,
    ...
    // KV cache expansion: try to grow the KV cache buffer to accommodate more tokens.
    // Returns true if the cache was successfully expanded.
    LLAMA_API bool llama_memory_expansion(
            llama_memory_t mem,
            struct llama_context * ctx,
                  uint32_t   n_tokens);
