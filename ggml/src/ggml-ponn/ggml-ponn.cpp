...
static void ggml_backend_ponn_buffer_free_buffer(
    ...
    for (auto * extra : ctx->tensor_extras) {
        ...
        if (extra->origin_quant_data != nullptr) {
            free(extra->origin_quant_data);
            extra->origin_quant_data = nullptr;
            extra->origin_quant_data_size = 0;
            extra->is_transform = false;
        }
#endif
        delete extra;
    }
 ...
 */
static ggml_status ggml_backend_ponn_buffer_init_tensor(
...
static void ggml_backend_ponn_buffer_set_tensor(
    ...
    if(need_transform(tensor->type)) {
        ...
        GGML_ASSERT(extra->origin_quant_data == nullptr);
        extra->origin_quant_data = malloc(size);
        extra->origin_quant_data_size = size;
        extra->is_transform = true;
        memcpy(extra->origin_quant_data, data, size);
#endif
        ...
        if(tensor->type == GGML_TYPE_F32){
            ...
            std::vector<int> tensor_dims;
            ponn_utils_get_tensor_dims(tensor_dims, tensor->ne);
            ponnMemcpyEx(extra->handle, PONN_DATA_S32, data, ponn_utils_get_data_type(tensor->type), size, tensor_dims, HOST_TO_DEVICE);
        }
        else {
...
static bool ggml_ponn_compute_forward(ggml_backend_ponn_context& ctx,
    ...
    switch (dst->op) {
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            ggml_ponn_flash_attn_ext(ctx, dst);
            break;
        case GGML_OP_SCALE:
        ...
        case GGML_OP_SSM_CONV:
            ggml_ponn_ssm_conv(ctx, dst);
            break;
        case GGML_OP_L2_NORM:
            ggml_ponn_l2_norm(ctx, dst);
            break;
        case GGML_OP_SUB:
            ggml_ponn_sub(ctx, dst);
            break;
        case GGML_OP_TRI:
            ggml_ponn_tri(ctx, dst);
            break;
        case GGML_OP_SOLVE_TRI:
            ggml_ponn_solve_tri(ctx, dst);
            break;
        case GGML_OP_FILL:
            ggml_ponn_fill(ctx, dst);
            break;
        case GGML_OP_DIAG:
            ggml_ponn_diag(ctx, dst);
            break;
        case GGML_OP_CUMSUM:
            ggml_ponn_cum_sum(ctx, dst);
            break;
        case GGML_OP_SET:
            ggml_ponn_set(ctx, dst);
            break;
        default:
            return false;
    }
...
static enum ggml_status ggml_backend_ponn_graph_compute(
	...
	//目前硬编码为cgraph每1/6 Sync一次，其中6=(115/20)+1
	if(i % (cgraph->n_nodes / 6) == 0) {
        ...
        if (node->op == GGML_OP_MUL_MAT && next_node && next_node->op == GGML_OP_SOFT_MAX) {
            ...
            if (ponn_is_same_tensor(extra0, extra1) && next_node->op_params[0]) {
                float scale = 0.0f;
                memcpy(&scale, (float *) next_node->op_params + 0, sizeof(float));
                if(scale != 1.0f) {
                    // set matmul scale = softmax scale
                    node->op_params[1] = next_node->op_params[0];
                    // softmax scale set 1
                    scale = 1.0f;
                    memcpy((float *) next_node->op_params + 0, &scale,    sizeof(float));
                }
        ...
        GGML_LOG_INFO("name %32.32s %10.10s  ", node->name, ggml_op_name(node->op));
...
static bool ggml_backend_ponn_supports_op(ggml_backend_dev_t dev,
    ...
    switch (op->op) {
        ...
        case GGML_OP_MUL_MAT_ID:
            return false;
        case GGML_OP_FLASH_ATTN_EXT:
            return ggml_ponn_flash_attn_canrun(op);
        // embedding
        case GGML_OP_GET_ROWS: {
            switch (op->src[0]->type) {
                case GGML_TYPE_F32:
...
static const ggml_backend_device_i ggml_backend_ponn_device_interface = {
    ...
    /* .buffer_from_host_ptr    = */ NULL, // not supported for PONN
    /* .supports_op             = */ ggml_backend_ponn_supports_op,
    /* .supports_buft           = */ ggml_backend_ponn_supports_buft,
    /* .offload_op              = */ ggml_backend_ponn_offload_op,
...
// backend registry, called only once for ponn backend
