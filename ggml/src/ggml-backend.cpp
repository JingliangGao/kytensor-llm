...
struct ggml_backend_sched {
    ...
    bool backend_is_rpp[GGML_SCHED_MAX_BACKENDS]; // 运行时按后端名缓存,避免热路径重复判断
...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    // Profiling: helper to append a copy record with the correct direction label
    auto push_copy_record = [&](int split_backend_id, int split_id, uint64_t start_ns, uint64_t end_ns,
                                uint64_t bytes, const struct ggml_tensor * input,
    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        const bool is_rpp = sched->backend_is_rpp[split_backend_id];

#ifdef LLAMA_USE_PROFILER
        // Profiling: set split ID and enable backend profiler
        if (sched->profiling_enabled) {
        ...
        for (int input_id = 0; input_id < split->n_inputs; input_id++) {
            ...
            if (input->flags & GGML_TENSOR_FLAG_INPUT) {
                ...
                if (is_rpp) {
                    // RPP: submit an async tensor set and synchronize once after the last input
...
#ifdef LLAMA_USE_PROFILER
                if (sched->profiling_enabled) {
                    push_copy_record(split_backend_id, split_id, copy_start, ggml_profiler_time_ns(),
                                     ggml_nbytes(input), input, input_backend, split_backend);
                }
#endif
            } else {
                ...
                if (split->graph.n_nodes > 0 &&
...
#ifdef LLAMA_USE_PROFILER
                    if (sched->profiling_enabled) {
                        push_copy_record(split_backend_id, split_id, moe_copy_start, ggml_profiler_time_ns(),
                                         (uint64_t) total_copied_bytes, input, input_backend, split_backend);
                    }
#endif
                } else {
                    ...
                    if (!split_backend->iface.cpy_tensor_async || !split_backend->iface.cpy_tensor_async(input_backend, split_backend, input, input_cpy)) {
                        ...
                        if (is_rpp) {
                            // RPP: submit an async tensor set and synchronize once after all inputs
...
ggml_backend_sched_t ggml_backend_sched_new(
    ...
    for (int b = 0; b < n_backends; b++) {
        ...
        sched->backend_is_rpp[b] = strncmp(ggml_backend_name(backends[b]), "RPP", 3) == 0;
