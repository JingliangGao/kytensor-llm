...
static RPPdeviceptr clamp_prepare_lut_workspace(rpp_kernel_context & ctx, float clamp_min, float clamp_max) {
    ...
    rpp_dev_lut_key key{};
    key.kind        = rpp_dev_lut_kind::clamp_lut;
    key.bytes       = (size_t) lut_size;
    key.params_hash = ((uint64_t) min_bits << 32) | (uint64_t) max_bits;
        ...
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [=](RPPdeviceptr workspace, size_t bytes) {
                ...
                for (uint32_t i = 0; i < (uint32_t) lut_elements; ++i) {
                    ...
                    clamp_table[i] =
                        rpp::bfloat16::round_to_bfloat16(std::max(std::min(x, clamp_max), clamp_min)).value;
                }
                if (rtMemcpy((void *) workspace, clamp_table.data(), bytes, rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize Clamp LUT");
                }
            });
