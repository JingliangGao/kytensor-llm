...
struct ggml_backend_rpp_context {
    ...
    /**
     * Stores the backend abort callback and its user data.
     *
     * @param callback Callback that returns true when inference must stop.
    ...
    /**
     * Releases backend-owned runtime objects and manager-owned static resources.
     *
     * The operation is idempotent and does not delete this context object.
     */
