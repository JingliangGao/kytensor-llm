...
static RPPdeviceptr rpp_flash_attn_prepare_lut_workspace(rpp_kernel_context & ctx, int lut_elements) {
    ...
    rpp_dev_lut_key key{};
    key.kind  = rpp_dev_lut_kind::flash_attn_lut;
    key.bytes = (size_t) total_bytes;

    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [=](RPPdeviceptr workspace, size_t) {
                ...
                for (uint32_t i = 0; i < (uint32_t) lut_elements; i++) {
                    ...
                    exp_table[i] = rpp::bfloat16::round_to_bfloat16(std::exp(value)).value;
                    div_table[i] = rpp::bfloat16::round_to_bfloat16(1.0f / value).value;
                }
                div_table[0]      = 0;
                div_table[0x8000] = 0;
                if (rtMemcpy((void *) workspace, exp_table.data(), lut_bytes, rtMemcpyHostToDevice) != rtSuccess ||
                    rtMemcpy((void *) (workspace + lut_bytes), div_table.data(), lut_bytes,
                             rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize FlashAttention LUT");
                }
            });
...
static int64_t flash_attn_full_path_sram_bytes(int Tq_tile,
    ...
    const int     Tk32          = (int) round_up_32((uint32_t) Tk);
    const bool    pad_t         = Tk32 != Tk;
    const int64_t bf16_bytes    = (int64_t) sizeof(rpp::bfloat16);
    const int64_t sizeQ         = (int64_t) Tq_tile * Dp * io_bytes_per_elem;
    const int64_t sizeQHw32     = (int64_t) Tq_tile * Dp * bf16_bytes;
    const int64_t sizeKRaw      = pad_t ? ((int64_t) Tk * Dp + 64) * bf16_bytes : 0;
    const int64_t sizeK         = (int64_t) Tk32 * Dp * kv_bytes_per_elem;
    const int64_t sizeKt        = (int64_t) Tk32 * (Dp + 1) * kv_bytes_per_elem;
    const int64_t sizeVRaw      = pad_t ? ((int64_t) Tk * Dp + 64) * bf16_bytes : 0;
    const int64_t sizeV         = (int64_t) Tk32 * Dp * kv_bytes_per_elem;
    const int64_t sizeMask      = (int64_t) Tq_tile * Tk * mask_bytes_per_elem +
                             (pad_t ? 64 * bf16_bytes : 0);
    const int64_t sizeMaskPad   = (int64_t) Tq_tile * Tk32 * bf16_bytes;
    const int64_t sizeMaskHw32  = sizeMaskPad;
    const int64_t sizeOBf16     = (int64_t) Tq_tile * Dp * bf16_bytes;
    const int64_t sizeOIo       = (int64_t) Tq_tile * Dp * io_bytes_per_elem;
    const int64_t sizeScore     = (int64_t) Tq_tile * Tk32 * bf16_bytes;
    const int64_t lutSize       = 64 * 1024 * (int64_t) sizeof(short);

    int64_t total = 0;
    total += round_up_sram_bytes(sizeQ);
    if (pad_t) {
        total += round_up_sram_bytes(sizeKRaw);
    }
    total += round_up_sram_bytes(sizeK);
    total += round_up_sram_bytes(sizeKt);
    if (pad_t) {
        total += round_up_sram_bytes(sizeVRaw);
    }
    total += round_up_sram_bytes(sizeV);
    total += round_up_sram_bytes(sizeMask);
    if (pad_t) {
        total += round_up_sram_bytes(sizeMaskPad);
    }
...
static void fill_sram_16bit_aligned(RPPdeviceptr            addr,
                                    int                     bytes,
                                    int16_t                 value,
                                    dim3 &                  threadsPerBlock,
                                    dim3 &                  blocksPerGrid,
...
/**
 * @brief 将 SRAM 中连续的 BF16/U16 [N,T,D] 数据复制到 [N,T32,D]，其中 T32 向上对齐到 32。
 *
 * @param input SRAM 输入地址，调用方需为末尾额外准备至少 64 个零值 guard 元素。
 * @param output SRAM 输出地址，调用前应初始化 padding 区域。
 * @param n_batches N 维批次数，必须不超过 16 位 grid.y 范围。
 * @param t 有效 T 长度，要求不是 32 的倍数。
 * @param d 每个 T 行的元素数。
 * @param module 已加载且包含 flash_attn_qkv_pad_t_32x 的 RPP module。
 * @param stream 执行 padding kernel 的 stream。
 */
static void launch_flash_attn_pad_t_32x(RPPdeviceptr input,
                                       RPPdeviceptr output,
                                       uint32_t     n_batches,
                                       uint32_t     t,
                                       uint32_t     d,
                                       RPPmodule    module,
                                       RPPstream    stream) {
    constexpr uint32_t kBulkThreads     = 8160;
    constexpr uint32_t kMinBlockThreads = 64;

    if (n_batches == 0 || n_batches > UINT16_MAX || t == 0 || d == 0 || (t % 32u) == 0) {
        throw std::runtime_error("invalid flash_attn_qkv_pad_t_32x shape");
    }

    const uint64_t valid_elems_u64 = (uint64_t) t * d;
    if (valid_elems_u64 > UINT32_MAX) {
        throw std::overflow_error("flash_attn_qkv_pad_t_32x element count overflow");
    }
    const uint32_t valid_elems = (uint32_t) valid_elems_u64;
    const uint32_t t32         = (t + 31u) & ~31u;
    const uint32_t src_stride  = valid_elems * (uint32_t) sizeof(uint16_t);
    const uint32_t dst_stride  = t32 * d * (uint32_t) sizeof(uint16_t);
    const uint32_t large_elems = valid_elems > kBulkThreads ?
                                     (valid_elems / kBulkThreads) * kBulkThreads :
                                 valid_elems >= kMinBlockThreads ? valid_elems :
                                                                  0;
    const uint32_t small_elems = valid_elems - large_elems;

    auto launch_copy = [&](RPPdeviceptr in_addr, RPPdeviceptr out_addr, uint32_t block_x, uint32_t grid_x) {
        if (block_x > UINT16_MAX || grid_x > UINT16_MAX) {
            throw std::overflow_error("flash_attn_qkv_pad_t_32x launch shape overflow");
        }
        dim3 threads_per_block{ block_x, 1, 1 };
        dim3 blocks_per_grid{ grid_x, n_batches, 1 };
        std::vector<uint32_t> pad_params;
        pad_params.emplace_back((uint32_t) in_addr);
        pad_params.emplace_back((uint32_t) out_addr);
        pad_params.emplace_back(src_stride);
        pad_params.emplace_back(dst_stride);
        launchWrapperAysnc("flash_attn_qkv_pad_t_32x", blocks_per_grid, threads_per_block, pad_params, module, stream);
    };

    if (large_elems != 0) {
        const uint32_t block_x = valid_elems > kBulkThreads ? kBulkThreads : valid_elems;
        launch_copy(input, output, block_x, large_elems / block_x);
    }
    if (small_elems != 0) {
        const uint32_t offset = large_elems * (uint32_t) sizeof(uint16_t);
        launch_copy(input + offset, output + offset, std::max(small_elems, kMinBlockThreads), 1);
    }
}
...
static void convert_reduce_task_to_reciprocal_f16(RppTaskElement & task, uint32_t reciprocal_table_addr) {
    if (task.taskName == "opt_gen_reduce_f16" || task.taskName == "gen_reduce_f16") {
...
static void rpp_flash_atten_build_impl(rpp_kernel_context & ctx,
    ...
    const int  Tk32         = round_up_32(Tk);
    const bool use_padded_d = (Dp != D);
    const bool use_padded_t = (Tk32 != Tk);
    ...
    const int    sizeKRaw     = use_padded_t ? (Tk * Dp + 64) * (int) sizeof(rpp::bfloat16) : 0;
    const int    sizeK        = Tk32 * Dp * kv_bytes_per_elem;
    // `blas_trans_padding` pads each K row to (D + 1) elements before transpose.
    const int    sizeKt       = Tk32 * (Dp + 1) * kv_bytes_per_elem;
    const int    sizeVRaw     = use_padded_t ? (Tk * Dp + 64) * (int) sizeof(rpp::bfloat16) : 0;
    const int    sizeV        = Tk32 * Dp * kv_bytes_per_elem;
    const int    sizeMask     = Tq_tile_capacity * Tk * mask_bytes_per_elem +
                            (use_padded_t ? 64 * (int) sizeof(rpp::bfloat16) : 0);
    const int    sizeMaskPad  = use_padded_t ? Tq_tile_capacity * Tk32 * (int) sizeof(rpp::bfloat16) : 0;
    const int    sizeMaskHw32 = Tq_tile_capacity * Tk32 * (int) sizeof(rpp::bfloat16);
    const int    sizeOBf16    = Tq_tile_capacity * Dp * (int) sizeof(rpp::bfloat16);
    const int    sizeOIo      = Tq_tile_capacity * Dp * io_bytes_per_elem;
    const int    sizeScore    = Tq_tile_capacity * Tk32 * (int) sizeof(rpp::bfloat16);
    const int    lutSize      = 64 * 1024 * sizeof(short);
    RPPdeviceptr sram_base    = ctx.virtual_sram_base;
    RPPdeviceptr sramQ        = sram_base;
    RPPdeviceptr sramKRaw     = sramQ + round_up(sizeQ);
    RPPdeviceptr sramK        = sramKRaw + (use_padded_t ? round_up(sizeKRaw) : 0);
    RPPdeviceptr sramKt       = sramK + round_up(sizeK);
    RPPdeviceptr sramVRaw     = sramKt + round_up(sizeKt);
    RPPdeviceptr sramV        = sramVRaw + (use_padded_t ? round_up(sizeVRaw) : 0);
    RPPdeviceptr sramMask     = sramV + round_up(sizeV);
    RPPdeviceptr sramMaskPad  = sramMask + round_up(sizeMask);
    RPPdeviceptr sramO        = sramMaskPad + (use_padded_t ? round_up(sizeMaskPad) : 0);
    ...
    for (int h = 0; h < Hq; h++) {
        ...
        if (h == 0 || kv_h != ((h - 1) / group)) {
            ...
            const RPPdeviceptr k_input_stage = use_padded_t ? sramKRaw : sramK;
            const RPPdeviceptr v_input_stage = use_padded_t ? sramVRaw : sramV;
            if (use_padded_t) {
                zero_fill_sram_16bit_aligned(sramKRaw, sizeKRaw, threadsPerBlock, blocksPerGrid, params, ctx.rppBinMod,
                                             ctx.kernelStream);
                zero_fill_sram_16bit_aligned(sramVRaw, sizeVRaw, threadsPerBlock, blocksPerGrid, params, ctx.rppBinMod,
                                             ctx.kernelStream);
                ...
                zero_fill_sram_16bit_aligned(sramV, sizeV, threadsPerBlock, blocksPerGrid, params, ctx.rppBinMod,
                                             ctx.kernelStream);
            } else if (use_padded_d) {
                zero_fill_sram_16bit_aligned(sramK, sizeK, threadsPerBlock, blocksPerGrid, params, ctx.rppBinMod,
                                             ctx.kernelStream);
                zero_fill_sram_16bit_aligned(sramV, sizeV, threadsPerBlock, blocksPerGrid, params, ctx.rppBinMod,
                                             ctx.kernelStream);
            }
            rtMemcpy2DAsync((void *) k_input_stage, Dp * sizeof(rpp::bfloat16), (const void *) devK_h,
                            D * Hkv * sizeof(rpp::bfloat16), D * sizeof(rpp::bfloat16), Tk, rtMemcpyDeviceToSram,
                            ctx.kernelStream);
            rtMemcpy2DAsync((void *) v_input_stage, Dp * sizeof(rpp::bfloat16), (const void *) devV_h,
                            D * Hkv * sizeof(rpp::bfloat16), D * sizeof(rpp::bfloat16), Tk, rtMemcpyDeviceToSram,
                            ctx.kernelStream);

            if (use_padded_t) {
                launch_flash_attn_pad_t_32x(k_input_stage, sramK, 1, (uint32_t) Tk, (uint32_t) Dp, ctx.rppBinMod,
                                            ctx.kernelStream);
                launch_flash_attn_pad_t_32x(v_input_stage, sramV, 1, (uint32_t) Tk, (uint32_t) Dp, ctx.rppBinMod,
                                            ctx.kernelStream);
            }
            // K Tranpose from [Tk, D] to K[D, Tk]

            params.clear();
            KShape_TransPadding(Tk32, Dp, threadsPerBlock, blocksPerGrid);
            ...
            params.clear();
            KShape_Trans(Tk32, Dp, threadsPerBlock, blocksPerGrid);
            KParam_Trans(threadsPerBlock, blocksPerGrid, sramKt, sramK, Tk32, Dp, params);
            launchWrapperAysnc("blas_transpose_u16", blocksPerGrid, threadsPerBlock, params, ctx.rppBinMod,
                               ctx.kernelStream);

            // K from HW to HW32
            launch_hw2hw32_transpose_f16(sramK, sramKr, Dp, Tk32, threadsPerBlock, blocksPerGrid, params,
                                         ctx.rppBinMod, ctx.kernelStream);

            // V from HW to HW32
            launch_hw2hw32_transpose_f16(sramV, sramVr, Tk32, Dp, threadsPerBlock, blocksPerGrid, params,
                                         ctx.rppBinMod, ctx.kernelStream);

            // V from HW to HW32
            params.clear();
            KShape_Hw2Hw32(Tk32, Dp, threadsPerBlock, blocksPerGrid);
            KParam_Hw2Hw32(threadsPerBlock, blocksPerGrid, sramV, sramVr, Tk32, Dp, params);
        ...
        for (int q_start = 0; q_start < Tq;) {
            ...
            const RPPdeviceptr sramMaskLinearStage = use_padded_t ? sramMaskPad : sramMask;
            ...
            if (has_mask) {
                if (use_padded_t) {
                    zero_fill_sram_16bit_aligned(sramMask, sizeMask, threadsPerBlock, blocksPerGrid, params,
                                                 ctx.rppBinMod, ctx.kernelStream);
                }
                rtMemcpyAsync((void *) sramMask, (const void *) (devMask + (size_t) q_start * mask_row_bytes),
                              sizeMaskTile, rtMemcpyDeviceToSram, ctx.kernelStream);
            ...
            } else if (use_padded_t) {
                zero_fill_sram_16bit_aligned(sramMask, TqTile * Tk * (int) sizeof(rpp::bfloat16), threadsPerBlock,
                                             blocksPerGrid, params, ctx.rppBinMod, ctx.kernelStream);
            }

            if (use_padded_t) {
                constexpr uint16_t bf16_negative_infinity = 0xFF80u;
                zero_fill_sram_16bit_aligned(
                    sramMaskBf16Stage + (RPPdeviceptr) TqTile * Tk * sizeof(rpp::bfloat16),
                    64 * (int) sizeof(rpp::bfloat16), threadsPerBlock, blocksPerGrid, params, ctx.rppBinMod,
                    ctx.kernelStream);
                for (int row = 0; row < TqTile; ++row) {
                    const RPPdeviceptr src_row =
                        sramMaskBf16Stage + (RPPdeviceptr) row * Tk * sizeof(rpp::bfloat16);
                    const RPPdeviceptr dst_row =
                        sramMaskPad + (RPPdeviceptr) row * Tk32 * sizeof(rpp::bfloat16);
                    launch_flash_attn_pad_t_32x(src_row, dst_row, 1, (uint32_t) Tk, 1, ctx.rppBinMod,
                                                ctx.kernelStream);
                    fill_sram_16bit_aligned(dst_row + (RPPdeviceptr) Tk * sizeof(rpp::bfloat16),
                                            (Tk32 - Tk) * (int) sizeof(rpp::bfloat16),
                                            (int16_t) bf16_negative_infinity, threadsPerBlock, blocksPerGrid, params,
                                            ctx.rppBinMod, ctx.kernelStream);
                }
            }

            if (has_mask || use_padded_t) {
                // Mask from HW to HW32
                launch_hw2hw32_transpose_f16(sramMaskLinearStage, sramMaskHw32Stage, TqTile, Tk32, threadsPerBlock,
                                             blocksPerGrid, params, ctx.rppBinMod, ctx.kernelStream);
            }
            ...
            int         first_mm_b_col = Tk32;
            std::string first_mm_kernel;
            const bool         apply_mask      = has_mask || use_padded_t;
            const RPPdeviceptr first_mm_output = apply_mask ? workspace0_addr : workspace1_addr;
            params.clear();
            create_mm_chw32_unroll(first_mm_a_row, first_mm_a_col, first_mm_b_row, first_mm_b_col, sramQr, sramKr,
                                   first_mm_output, first_mm_kernel, threadsPerBlock, blocksPerGrid, params);
            launchWrapperAysnc(first_mm_kernel, blocksPerGrid, threadsPerBlock, params, ctx.rppBinMod,
                               ctx.kernelStream);

            if (apply_mask) {
                // Add with Mask
                int add_block_len  = Tk32 * TqTile;
                int add_repeat_num = 1;
                while (add_block_len >= 8192) {
                    add_block_len /= 2;
                    add_repeat_num *= 2;
                }
                assert(add_repeat_num * add_block_len == Tk32 * TqTile);
            ...
            for (size_t j = 0; j < second_reduce_sum_io_dims.size() - 1; j++) {
                ...
                for (RppTaskElement & task : tasks) {
                    ...
                    if (j == second_reduce_sum_io_dims.size() - 2) {
                        convert_reduce_task_to_reciprocal_f16(task, reciprocal_table_addr);
                        launchWrapperAysnc(task.taskName, task.gridDim, task.blockDim, task.params.kernelList,
                                           ctx.rppBinMod, ctx.kernelStream);
                    } else {
                        launchWrapperAysnc(task.taskName, task.gridDim, task.blockDim, task.params.kernelList,
            ...
            int second_mm_a_col = Tk32;
            int second_mm_b_row = Tk32;
        ...
        if (has_mask) {
            for (int row_base = 0; row_base < tile_hq; row_base += loop2_rows_per_launch) {
                ...
                loop2_mid_add_bc_y_f32_f16_f32_params(
                    (uint32_t) bank.sramScoreHighAll + row_offset_bytes,
                    (uint32_t) bank.sramScoreLowAll + row_offset_bytes, (uint32_t) sramMaskStage,
                    (uint32_t) bank.sramScoreHighAll + row_offset_bytes,
                    (uint32_t) bank.sramScoreLowAll + row_offset_bytes, kv_page * sizeof(uint16_t),
                    score_row_stride_bytes * (uint32_t) tile_rows, score_row_stride_elems,
                    kv_page * sizeof(uint16_t), 1, (uint32_t) kv_page_repeat, add_task.params.kernelList);
                add_task.taskName = "llama3_loop2_mid_add_bc_y_f32_f16_f32";
                launchWrapperAysnc(add_task.taskName, add_task.gridDim, add_task.blockDim, add_task.params.kernelList,
                                   ctx.rppBinMod, ctx.kernelStream);
            }
...
void rpp_flash_atten_build_vxm_v1(rpp_kernel_context & ctx,
    ...
    if (Tk > FLASH_VXM_SMALL_KV_THRESHOLD) {
        rpp_flash_atten_build_vxm(ctx, Tq, Tk, Hq, Hkv, D, scale, kv_bytes_per_elem, io_bytes_per_elem,
                                  mask_bytes_per_elem, has_mask, is_instantial);
        return;
    }
    if (kv_bytes_per_elem != sizeof(rpp::bfloat16)) {
        throw std::runtime_error("Not Supportted KV Storage Type");
    ...
    if (has_mask && mask_bytes_per_elem == sizeof(float)) {
        params.clear();
        calc_tbdim_flattern(1, Tk, threadsPerBlock, blocksPerGrid);
        cvt_kernel_param_init(threadsPerBlock, sramMaskRaw, sramMaskBf16, kFLOAT, kBF16, params);
        launchWrapperAysnc("opt_vector_cvt_32_16", blocksPerGrid, threadsPerBlock, params, ctx.rppBinMod,
    ...
    if (has_mask) {
        for (int row_base = 0; row_base < Hq; row_base += loop2_rows_per_launch) {
            const int tile_rows =
                (((row_base + loop2_rows_per_launch) <= Hq) ? loop2_rows_per_launch : (Hq - row_base));
            const uint32_t row_offset_bytes = (uint32_t) row_base * score_row_stride_bytes;
...
static void fill_sram_16bit_aligned(RPPdeviceptr            addr,
    ...
    params.clear();
    constexpr int max_threads = 8160;
    const int block_x         = std::min(std::max(count16, 33), max_threads);
    const int grid_x          = (count16 + block_x - 1) / block_x;
    if (grid_x > (int) UINT16_MAX) {
        throw std::overflow_error("flash attention SRAM fill grid exceeds supported range");
    }
    threadsPerBlock = dim3((uint32_t) block_x, 1, 1);
    blocksPerGrid   = dim3((uint32_t) grid_x, 1, 1);
    fill_16bits_align_params(
        (int) addr, (int) threadsPerBlock.x, value, (int) sizeof(uint16_t), params);
...
static void zero_fill_sram_16bit_aligned(RPPdeviceptr            addr,
                                         ...
                                         RPPmodule &             module,
                                         rtStream_t              stream) {
    fill_sram_16bit_aligned(addr, bytes, 0, threadsPerBlock, blocksPerGrid, params, module, stream);
}
