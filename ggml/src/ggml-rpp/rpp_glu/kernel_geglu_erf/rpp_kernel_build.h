...
static RPPdeviceptr gelu_prepare_lut_workspace_once(rpp_kernel_context & ctx, int gelu_variant, int lut_elements) {
    ...
    rpp_dev_lut_key key{};
    key.kind    = rpp_dev_lut_kind::gelu_lut;
    key.variant = (uint32_t) gelu_variant;
    key.bytes   = lut_bytes;
    ...
    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [=](RPPdeviceptr workspace, size_t bytes) {
                ...
                if (rtMemcpy((void *) workspace, gelu_table.data(), bytes, rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize GELU LUT");
                }
            });
