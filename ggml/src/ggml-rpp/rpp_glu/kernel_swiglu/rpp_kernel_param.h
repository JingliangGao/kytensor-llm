...
static inline RPPdeviceptr silu_prepare_lut_workspace(rpp_kernel_context & ctx) {
    ...
    rpp_dev_lut_key key{};
    key.kind  = rpp_dev_lut_kind::silu_lut;
        ...
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [](RPPdeviceptr workspace, size_t bytes) {
                ...
                for (uint32_t i = 0; i < lut_elements; ++i) {
                    ...
                    silu_table[i] =
                        rpp::bfloat16::round_to_bfloat16(x / (1.0f + std::exp(-x))).value;
                }
                if (rtMemcpy((void *) workspace, silu_table.data(), bytes, rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize SiLU LUT");
                }
            });
