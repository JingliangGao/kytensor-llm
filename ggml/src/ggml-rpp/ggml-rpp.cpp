...
void ggml_backend_rpp_context::set_abort_callback(ggml_abort_callback callback, void * data) {
    std::lock_guard<std::mutex> lock(abort_mutex);
    abort_callback      = callback;
    abort_callback_data = data;
}

bool ggml_backend_rpp_context::should_abort() {
    if (resources_released.load(std::memory_order_acquire)) {
        return true;
    }

    ggml_abort_callback callback = nullptr;
    void *              data     = nullptr;
    {
        std::lock_guard<std::mutex> lock(abort_mutex);
        callback = abort_callback;
        data     = abort_callback_data;
    }

    return callback != nullptr && callback(data);
}

void ggml_backend_rpp_context::release_resources() {
    ...
    ggml_rpp_set_device(device);
    (void) rtDeviceSynchronize();

    cur_rpp_graph = nullptr;
    rpp_graphs.clear();
    gglm_graphs.clear();
    rpp_io_buffers.clear();
    ggml_first_properties.clear();

    if (!rpp_dev_resource_manager::instance().release_device(device)) {
        GGML_LOG_WARN("%s: failed to release one or more static resources on device %d\n", __func__, device);
    }
    ...
    for (int i = 0; i < GGML_RPP_MAX_DEVICES; ++i) {
        ...
        pools_mem[i].reset();
        pools_host[i].reset();
        pools_host_leg[i].reset();
        pools_host_mem[i].reset();
    }

    if (copy_event != nullptr) {
        (void) rtEventDestroy(copy_event);
        copy_event = nullptr;
    }
    for (int i = 0; i < GGML_RPP_MAX_DEVICES; ++i) {
        for (int j = 0; j < GGML_RPP_MAX_STREAMS; ++j) {
            if (streams[i][j] != nullptr) {
                (void) rtStreamDestroy(streams[i][j]);
                streams[i][j] = nullptr;
            }
        }
    }

    resources_released.store(true, std::memory_order_release);
}
...
struct ggml_backend_rpp_buffer_context {
    ...
    std::string name;
    rtStream_t  upload_stream = nullptr;
    ...
    ggml_backend_rpp_buffer_context(int device, void * dev_ptr) :
        ...
        ggml_rpp_set_device(device);
        RPP_CHECK(rtStreamCreateWithFlags(&upload_stream, rtStreamNonBlocking));
    }

    ~ggml_backend_rpp_buffer_context();
};
...
/**
 * Builds a unique in-memory index key for a cache entry.
 *
 * Multiple weights with identical metadata, such as two LoRA adapters, share
 * the same tensor_key but have different input fingerprints. Including the
 * input size, output size, and fingerprint ensures that each weight resolves
 * to its own cache entry.
 *
 * @param tensor_key Base key derived from the tensor name, type, dimensions,
 *                   and strides.
 * @param input_size Original weight data size in bytes.
 * @param output_size Converted weight data size in bytes.
 * @param input_fingerprint Fingerprint of the original weight data.
 * @return In-memory index key that uniquely identifies the weight contents.
 */
static std::string ggml_rpp_weights_cache_make_index_key(const std::string & tensor_key,
                                                         uint64_t            input_size,
                                                         uint64_t            output_size,
                                                         uint64_t            input_fingerprint) {
    std::string index_key = tensor_key;
    index_key.reserve(index_key.size() + 96);
    index_key += "|input_size=" + std::to_string(input_size);
    index_key += "|output_size=" + std::to_string(output_size);
    index_key += "|input_fingerprint=" + std::to_string(input_fingerprint);
    return index_key;
}
...
/**
 * Resolves the RPP weight cache path.
 *
 * GGML_RPP_WEIGHTS_CACHE_FILE takes precedence. When it is unset or empty,
 * the default path is /var/cache/rpp/model_cache.weights.
 *
 * @return Resolved cache file path.
 */
static std::string ggml_rpp_weights_cache_resolve_path() {
    const char * configured_path = getenv("GGML_RPP_WEIGHTS_CACHE_FILE");
    if (configured_path != nullptr && configured_path[0] != '\0') {
        return configured_path;
    }

    return "/var/cache/rpp/model_cache.weights";
}

/**
 * Creates every missing parent directory for a cache file path.
 *
 * @param file_path Cache file path whose parent directories must exist.
 * @return True when all parent directories exist or were created.
 */
static bool ggml_rpp_weights_cache_create_parent_dirs(const std::string & file_path) {
    const size_t last_separator = file_path.find_last_of('/');
    if (last_separator == std::string::npos || last_separator == 0) {
        return true;
    }

    const std::string parent      = file_path.substr(0, last_separator);
    struct stat       parent_info = {};
    if (stat(parent.c_str(), &parent_info) == 0) {
        if (S_ISDIR(parent_info.st_mode)) {
            return true;
        }
        GGML_LOG_ERROR("%s: RPP weight cache parent path is not a directory: %s\n", __func__, parent.c_str());
        return false;
    }
    if (errno != ENOENT) {
        GGML_LOG_ERROR("%s: failed to inspect RPP weight cache directory %s: %s\n", __func__, parent.c_str(),
                       std::strerror(errno));
        return false;
    }
...
static void ggml_rpp_weights_cache_init_locked(ggml_rpp_weights_cache_state & cache_state) {
    ...
    cache_state.file_path = ggml_rpp_weights_cache_resolve_path();
    if (cache_state.file_path.empty()) {
        return;
    }

    std::ifstream in(cache_state.file_path, std::ios::binary);
    if (!in.is_open()) {
        if (!ggml_rpp_weights_cache_create_parent_dirs(cache_state.file_path)) {
            return;
        }
    ...
    while (true) {
        ...
        const std::string index_key = ggml_rpp_weights_cache_make_index_key(
            key, entry_header.input_size, entry_header.output_size, entry_header.input_fingerprint);
...
static bool ggml_rpp_weights_cache_try_load(const ggml_tensor * tensor,
    ...
    const std::string tensor_key  = ggml_rpp_weights_cache_make_key(tensor);
    const uint64_t    fingerprint = ggml_rpp_weights_cache_hash_bytes(input_data, input_size);
    const std::string index_key =
        ggml_rpp_weights_cache_make_index_key(tensor_key, input_size, output_size, fingerprint);

    auto it = cache_state.index.find(index_key);
...
static void ggml_rpp_weights_cache_store(const ggml_tensor * tensor,
    ...
    if (tensor_key.size() > (size_t) std::numeric_limits<uint32_t>::max()) {
        return;
    }
    ...
    ggml_rpp_weights_cache_entry_header entry_header = {};
    entry_header.key_size                            = (uint32_t) tensor_key.size();
    ...
    out.write(tensor_key.data(), (std::streamsize) tensor_key.size());
...
ggml_backend_rpp_buffer_context::~ggml_backend_rpp_buffer_context() {
    ggml_rpp_set_device(device);
    if (upload_stream != nullptr) {
        (void) rtStreamSynchronize(upload_stream);
        ggml_rpp_weights_cache_staging_free();
        (void) rtStreamDestroy(upload_stream);
...
static void ggml_backend_rpp_buffer_memset_tensor(ggml_backend_buffer_t buffer,
    ...
    ggml_rpp_set_device(ctx->device);
    RPP_CHECK(rtMemsetAsync((char *) tensor->data + offset, value, size, ctx->upload_stream));
...
static bool ggml_backend_rpp_buffer_cpy_tensor(ggml_backend_buffer_t buffer,
    ...
    if (ggml_backend_buffer_is_rpp(src->buffer)) {
        ...
        if (src_ctx->device == dst_ctx->device) {
            RPP_CHECK(
                rtMemcpyAsync(dst->data, src->data, ggml_nbytes(src), rtMemcpyDeviceToDevice, dst_ctx->upload_stream));
        } else {
            return false;
        }
        RPP_CHECK(rtStreamSynchronize(dst_ctx->upload_stream));
        return true;
    }
    return false;
...
static void ggml_backend_rpp_buffer_clear(ggml_backend_buffer_t buffer, uint8_t value) {
    ...
    ggml_rpp_set_device(ctx->device);
    RPP_CHECK(rtMemsetAsync(ctx->dev_ptr, value, buffer->size, ctx->upload_stream));
    RPP_CHECK(rtStreamSynchronize(ctx->upload_stream));
}
...
#    define GGML_RPP_TRACE_OP_CALL(call_expr)                                                                \
        do {                                                                                                 \
            const std::string trace_name = std::string("ggml_rpp_compute_forward:") + ggml_op_name(dst->op); \
 ...
 *
 * This function releases backend-owned and manager-owned resources before
 * deleting the backend context. It does not reset the RPP device.
 *
...
/**
 * Waits for all backend streams created for the active RPP Dev.
 *
 * @param rpp_ctx Backend context whose streams must complete.
 */
static void ggml_rpp_synchronize_backend_streams(ggml_backend_rpp_context * rpp_ctx) {
...
static void ggml_backend_rpp_synchronize(ggml_backend_t backend) {
    ggml_backend_rpp_context * rpp_ctx = (ggml_backend_rpp_context *) backend->context;
    ggml_rpp_synchronize_backend_streams(rpp_ctx);
    GGML_UNUSED(backend);
}
...
/**
 * Finishes already submitted Device work and returns the GGML abort status.
 *
 * @param rpp_ctx Backend context handling the abort.
 * @return GGML_STATUS_ABORTED.
 */
static enum ggml_status ggml_rpp_finish_aborted_compute(ggml_backend_rpp_context * rpp_ctx) {
    ggml_rpp_set_device(rpp_ctx->device);
    RPP_CHECK(rtDeviceSynchronize());
    GGML_LOG_INFO("%s: RPP graph computation aborted by callback\n", __func__);
    return GGML_STATUS_ABORTED;
}
                    ...
                    if (!disable_fusion) {
                        ...
                        if (node->op == GGML_OP_SOFT_MAX) {
                            ...
                            if (ggml_rpp_can_fuse_expert_routing(cgraph, i, routing_fusion,
                                ...
                                const bool ok_fuse = ggml_rpp_op_expert_routing_fusion(*rpp_ctx, routing_fusion,
                                                                                       is_instantial, is_launch);
                                GGML_ASSERT(ok_fuse);
                        ...
                        if (node->op == GGML_OP_MUL_MAT_ID) {
                            ...
                            if (ggml_rpp_can_fuse_expert_forward(cgraph, i, gate_tensor, up_tensor, down_tensor,
                                ...
                                const bool ok_fuse =
                                    ggml_rpp_op_export_forward(*rpp_ctx, gate_tensor, up_tensor, down_tensor,
                                                               div_tensor, add_tensor, is_instantial, is_launch);
                                GGML_ASSERT(ok_fuse);
                                continue;
                            }
                        }
            ...
            if (!disable_graph_capture) {
                ...
                GGML_ASSERT(ok_launch_funcs);
                {
                    ...
                    for (auto & cur_graph : rpp_in_use_kernel_graphs) {
                        if (rpp_ctx->should_abort()) {
                            return ggml_rpp_finish_aborted_compute(rpp_ctx);
                        }
...
static bool ggml_backend_rpp_device_supports_op(ggml_backend_dev_t dev, const ggml_tensor * op) {
    ...
    switch (op->op) {
        ...
        case GGML_OP_ROPE:
            {
                ...
                supported                = src0 && supports_rope_type(src0->type) && supports_rope_type(op->type) &&
                            src0->nb[0] == ggml_type_size(src0->type) && src0->nb[1] >= src0->ne[0] * src0->nb[0] &&
                            src0->nb[2] >= src0->ne[1] * src0->nb[1] && op->nb[0] == ggml_type_size(op->type) &&
                            op->nb[1] >= op->ne[0] * op->nb[0] && op->nb[2] >= op->ne[1] * op->nb[1];
            }
            break;
        case GGML_OP_CONT:
            supported = true;
        ...
        case GGML_OP_CLAMP:
            supported = op->src[0] != nullptr &&
                        (op->src[0]->type == GGML_TYPE_F32 || op->src[0]->type == GGML_TYPE_BF16) &&
                        (op->type == GGML_TYPE_F32 || op->type == GGML_TYPE_BF16) && op->src[0]->type == op->type &&
                        ggml_is_contiguous_1(op->src[0]) && ggml_is_contiguous_1(op);
            break;
        case GGML_OP_CONCAT:
...
static void * ggml_backend_rpp_reg_get_proc_address(ggml_backend_reg_t reg, const char * name) {
    ...
    if (strcmp(name, "ggml_backend_set_abort_callback") == 0) {
        return (void *) ggml_backend_rpp_set_abort_callback;
    }
    return nullptr;
}
