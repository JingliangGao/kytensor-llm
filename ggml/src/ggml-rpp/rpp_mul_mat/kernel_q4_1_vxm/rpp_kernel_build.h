...
static RPPdeviceptr q4_1_vxm_prepare_lut_workspace(rpp_kernel_context & ctx, int lut_elements) {
    ...
    key.variant = (uint32_t) GGML_TYPE_Q4_1 | RPP_DEV_LUT_VARIANT_VXM_LAYOUT;
    ...
    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [=](RPPdeviceptr workspace, size_t bytes) {
                ...
                if (rtMemcpy((void *) workspace, quant_table.data(), bytes, rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize Q4_1 VXM LUT");
                }
            });
