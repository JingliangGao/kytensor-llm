...
static void q2s_prepare_lut_workspace_once(rpp_kernel_context & ctx,
    ...
    key.variant = (uint32_t) GGML_TYPE_IQ2_S;
    key.bytes   = (size_t) lut_workspace_bytes;
    ...
    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [=](RPPdeviceptr workspace, size_t) {
                ...
                if (rtMemcpy((void *) workspace, qscale_lut.data(), qscale_lut_bytes,
                             rtMemcpyHostToDevice) != rtSuccess ||
                    rtMemcpy((void *) (workspace + qscale_lut_bytes), grid_lut.data(), grid_lut_bytes,
                             rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize Q2S LUT");
                }
            });
    RPPdeviceptr dev_lut_workspace =
        rpp_bind_managed_lut_workspace(ctx, managed_workspace, key.bytes);
...
static void rpp_matmul_q2s_build(rpp_kernel_context & ctx,
    ...
    q2s_prepare_lut_workspace_once(ctx, qscale_lut_bytes, grid_lut_bytes, devB_qscale_lut, devB_grid_lut);

    rppStreamBeginCapture(ctx.kernelStream, RPP_STREAM_CAPTURE_MODE_GLOBAL);
    rpp_module_load_once(ctx.rppBinMod, "rpp_kernel/matmul_q2s.o");
