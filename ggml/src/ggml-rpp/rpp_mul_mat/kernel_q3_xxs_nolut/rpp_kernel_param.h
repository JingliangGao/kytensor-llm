...
static void q3xxs_nolut_super_scale_params(uint32_t                in_scale,
...
static void q3xxs_nolut_dequant_params(dim3 &                  blocksPerGrid,
                                       dim3 &                  threadsPerBlock,
                                       uint32_t                in_wq,
                                       uint32_t                in_sign,
                                       uint32_t                scale,
    ...
    //----------------------------------------------------------------------------------------------------
    // codebook_nolut [K/256]      |  [8]  |  [2]     |  [3]      | [N]
    // codebook_nolut [z]          |  [y]  |  [loop]  |  [unroll] | [x]
    //----------------------------------------------------------------------------------------------------
    ...
    //----------------------------------------------------------------------------------------------------
    // out_addr      [K/256]       |  [8]  |  [2]     |  [16]      | [N]
    // out_addr      [z]           |  [y]  |  [loop]  |  [unroll]  | [x]
    //----------------------------------------------------------------------------------------------------
...
static inline int q3xxs_nolut_round_up(int a) {
    return (a + 511) / 512 * 512 + 512;
}

struct q3xxs_nolut_lut_workspace {
    // The current q3xxs no-LUT dequant kernel consumes qscale + mag LUTs.
    // The larger mat_lut used by older host-side prep is not referenced here.
    static constexpr uint32_t qscale_lut_elems = 16;
    static constexpr uint32_t mag_lut_elems    = 8;
...
// Full-tensor SRAM-direct layout for q3xxs_nolut:
...
// 3. Copy LUT workspace to SRAM via q3xxs_nolut_copy_lut_workspace_to_sram().
// 4. Launch rpp_matmul_q3xxs_nolut_sram().
// 5. Read final output from sram_out or use q3xxs_nolut_cdma_copy_output_to_ddr().
static inline q3xxs_nolut_sram_io q3xxs_nolut_prepare_sram_io(rpp_kernel_context & ctx,
                                                              int                  M,
                                                              int                  K,
                                                              int                  N,
                                                              int                  weights_group,
    ...
    if (M <= 0 || K <= 0 || N <= 0) {
        throw std::runtime_error("Q3XXS NoLUT SRAM expects positive M/K/N");
    }
    if ((K % 16) != 0) {
        throw std::runtime_error("Q3XXS NoLUT SRAM expects K % 16 == 0");
    }
    if ((N % 32) != 0) {
        throw std::runtime_error("Q3XXS NoLUT SRAM currently expects N % 32 == 0");
    }
    if (weights_group != kSuperGroup || (K % weights_group) != 0) {
        throw std::runtime_error("Q3XXS NoLUT SRAM expects weights_group == 256 and K % 256 == 0");
    }

    q3xxs_nolut_sram_io io{};
    io.M                     = M;
    io.K                     = K;
    io.N                     = N;
    ...
    io.sizeB_codebook_nolut = (uint32_t) (((uint64_t) K / 16ull) * 3ull * (uint64_t) N * sizeof(uint16_t));
    io.sizeB_scales         = (uint32_t) ((uint64_t) K * (uint64_t) N / 64ull);
    ...
    io.sizeB_scale_scratch  = (uint32_t) ((uint64_t) K * (uint64_t) N / 32ull * sizeof(uint16_t));
    ...
    io.sramA_out            = io.sramA_in + (RPPdeviceptr) q3xxs_nolut_round_up((int) io.sizeA_in);
    io.sramB_codebook_nolut = io.sramA_out + (RPPdeviceptr) q3xxs_nolut_round_up((int) io.sizeA_bf16);
    ...
    io.sramB_qscale_lut = io.sramB_codebook_nolut + (RPPdeviceptr) q3xxs_nolut_round_up((int) sizeB_weights_packed);
    io.sramB_mag_lut =
        io.sramB_qscale_lut + (RPPdeviceptr) q3xxs_nolut_round_up((int) q3xxs_nolut_lut_workspace::qscale_lut_bytes);
    io.sramB_scale =
        io.sramB_mag_lut + (RPPdeviceptr) q3xxs_nolut_round_up((int) q3xxs_nolut_lut_workspace::mag_lut_bytes);
    io.sramB_out      = io.sramB_scale + (RPPdeviceptr) q3xxs_nolut_round_up((int) io.sizeB_scale_scratch);
    io.sramC_hw32     = io.sramB_out + (RPPdeviceptr) q3xxs_nolut_round_up((int) io.sizeB_dequant);
    io.sramC_chw      = io.sramC_hw32 + (RPPdeviceptr) q3xxs_nolut_round_up((int) io.sizeC_bf16);
    io.sramC_chw_fp32 = io.sramC_chw + (RPPdeviceptr) q3xxs_nolut_round_up((int) io.sizeC_bf16);
    io.sram_out       = (out_bytes_per_element == (int) sizeof(float)) ? io.sramC_chw_fp32 : io.sramC_chw;
    io.total_sram_bytes =
        (uint32_t) ((io.sramC_chw_fp32 + (RPPdeviceptr) q3xxs_nolut_round_up((int) io.sizeC32)) - io.sram_base);
...
static inline RPPdeviceptr q3xxs_nolut_prepare_lut_workspace(rpp_kernel_context & ctx) {
    rpp_dev_lut_key key{};
    key.kind    = rpp_dev_lut_kind::quant_lut;
    key.variant = (uint32_t) GGML_TYPE_IQ3_XXS | RPP_DEV_LUT_VARIANT_NOLUT;
    key.bytes   = q3xxs_nolut_lut_workspace::total_bytes;
    ...
    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [](RPPdeviceptr workspace, size_t) {
                std::array<uint16_t, q3xxs_nolut_lut_workspace::qscale_lut_elems> qscale_lut = {};
                for (uint32_t i = 0; i < qscale_lut.size(); ++i) {
                    qscale_lut[i] = float_to_bf16_rne((0.5f + (float) i) * 0.5f);
                }
                constexpr std::array<float, q3xxs_nolut_lut_workspace::mag_lut_elems> values = {
                    4.0f, 12.0f, 20.0f, 28.0f, 36.0f, 44.0f, 52.0f, 62.0f,
                };
                std::array<uint16_t, q3xxs_nolut_lut_workspace::mag_lut_elems> mag_lut = {};
                for (uint32_t i = 0; i < mag_lut.size(); ++i) {
                    mag_lut[i] = float_to_bf16_rne(values[i]);
                }
                if (rtMemcpy((void *) workspace, qscale_lut.data(),
                             q3xxs_nolut_lut_workspace::qscale_lut_bytes, rtMemcpyHostToDevice) != rtSuccess ||
                    rtMemcpy((void *) (workspace + q3xxs_nolut_lut_workspace::qscale_lut_bytes), mag_lut.data(),
                             q3xxs_nolut_lut_workspace::mag_lut_bytes, rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize Q3XXS NoLUT LUT");
                }
            });
    return rpp_bind_managed_lut_workspace(ctx, managed_workspace, key.bytes);
}

static inline void q3xxs_nolut_cdma_d2s_async(RPPdeviceptr sram_dst,
...
static inline void q3xxs_nolut_cdma_s2d_async(RPPdeviceptr ddr_dst,
...
static inline void q3xxs_nolut_copy_lut_workspace_to_sram(const q3xxs_nolut_sram_io & io,
                                                          RPPdeviceptr                dev_lut_workspace,
                                                          RPPstream                   stream) {
    q3xxs_nolut_cdma_d2s_async(io.sramB_qscale_lut, dev_lut_workspace, q3xxs_nolut_lut_workspace::qscale_lut_bytes,
                               stream);
    q3xxs_nolut_cdma_d2s_async(io.sramB_mag_lut, dev_lut_workspace + q3xxs_nolut_lut_workspace::qscale_lut_bytes,
                               q3xxs_nolut_lut_workspace::mag_lut_bytes, stream);
}

static inline void q3xxs_nolut_cdma_copy_output_to_ddr(const q3xxs_nolut_sram_io & io,
                                                       RPPdeviceptr                devC,
                                                       RPPstream                   stream) {
    q3xxs_nolut_cdma_s2d_async(devC, io.sram_out, (size_t) io.sizeC, stream);
}
