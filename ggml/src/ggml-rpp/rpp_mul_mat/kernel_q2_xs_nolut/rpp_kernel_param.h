...
static void q2xs_nolut_dequant_params(dim3 &                  blocksPerGrid,
                                      dim3 &                  threadsPerBlock,
                                      uint32_t                in_wq,
                                      uint32_t                in_sign,
                                      uint32_t                scale,
    ...
    //            [grid.y]*[z]  |  [y]               |  [grid.x]*[x]
    //----------------------------------------------------------------------------------------------------
    // qsign/codebook_nolut/scale share same strideY & strideZ
...
static inline int q2xs_nolut_round_up(int a) {
    return (a + 511) / 512 * 512 + 512;
}

struct q2xs_nolut_lut_workspace {
    // The current q2xs no-LUT dequant kernel consumes qscale + mag LUTs.
...
// Full-tensor SRAM-direct layout for q2xs_nolut:
...
// 3. Copy LUT workspace to SRAM via q2xs_nolut_copy_lut_workspace_to_sram().
// 4. Launch rpp_matmul_q2xs_nolut_sram().
// 5. Read final output from sram_out or use q2xs_nolut_cdma_copy_output_to_ddr().
static inline q2xs_nolut_sram_io q2xs_nolut_prepare_sram_io(rpp_kernel_context & ctx,
                                                            int                  M,
                                                            int                  K,
                                                            int                  N,
                                                            int                  weights_group,
    ...
    if (M <= 0 || K <= 0 || N <= 0) {
        throw std::runtime_error("Q2XS NoLUT SRAM expects positive M/K/N");
    }
    if ((N % 32) != 0) {
        throw std::runtime_error("Q2XS NoLUT SRAM currently expects N % 32 == 0");
    }
    if (weights_group != kSuperGroup || (K % weights_group) != 0) {
        throw std::runtime_error("Q2XS NoLUT SRAM expects weights_group == 256 and K % 256 == 0");
    }

    q2xs_nolut_sram_io io{};
    io.M                     = M;
    io.K                     = K;
    io.N                     = N;
    ...
    io.sramA_out            = io.sramA_in + (RPPdeviceptr) q2xs_nolut_round_up((int) io.sizeA_in);
    io.sramB_codebook_nolut = io.sramA_out + (RPPdeviceptr) q2xs_nolut_round_up((int) io.sizeA_bf16);
    ...
    io.sramB_qscale_lut = io.sramB_codebook_nolut + (RPPdeviceptr) q2xs_nolut_round_up((int) sizeB_weights_packed);
    io.sramB_mag_lut =
        io.sramB_qscale_lut + (RPPdeviceptr) q2xs_nolut_round_up((int) q2xs_nolut_lut_workspace::qscale_lut_bytes);
    io.sramB_scale =
        io.sramB_mag_lut + (RPPdeviceptr) q2xs_nolut_round_up((int) q2xs_nolut_lut_workspace::mag_lut_bytes);
    io.sramB_out      = io.sramB_scale + (RPPdeviceptr) q2xs_nolut_round_up((int) io.sizeB_scale_scratch);
    io.sramC_hw32     = io.sramB_out + (RPPdeviceptr) q2xs_nolut_round_up((int) io.sizeB_dequant);
    io.sramC_chw      = io.sramC_hw32 + (RPPdeviceptr) q2xs_nolut_round_up((int) io.sizeC_bf16);
    io.sramC_chw_fp32 = io.sramC_chw + (RPPdeviceptr) q2xs_nolut_round_up((int) io.sizeC_bf16);
    io.sram_out       = (out_bytes_per_element == (int) sizeof(float)) ? io.sramC_chw_fp32 : io.sramC_chw;
    io.total_sram_bytes =
        (uint32_t) ((io.sramC_chw_fp32 + (RPPdeviceptr) q2xs_nolut_round_up((int) io.sizeC32)) - io.sram_base);
...
static inline RPPdeviceptr q2xs_nolut_prepare_lut_workspace(rpp_kernel_context & ctx) {
    rpp_dev_lut_key key{};
    key.kind    = rpp_dev_lut_kind::quant_lut;
    key.variant = (uint32_t) GGML_TYPE_IQ2_XS | RPP_DEV_LUT_VARIANT_NOLUT;
    key.bytes   = q2xs_nolut_lut_workspace::total_bytes;
    ...
    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [](RPPdeviceptr workspace, size_t) {
                std::array<uint16_t, q2xs_nolut_lut_workspace::qscale_lut_elems> qscale_lut = {};
                for (uint32_t i = 0; i < qscale_lut.size(); ++i) {
                    qscale_lut[i] = float_to_bf16_rne((0.5f + (float) i) * 0.25f);
                }
                constexpr std::array<float, q2xs_nolut_lut_workspace::mag_lut_elems> values = {
                    8.0f, 25.0f, 43.0f, 0.0f,
                };
                std::array<uint16_t, q2xs_nolut_lut_workspace::mag_lut_elems> mag_lut = {};
                for (uint32_t i = 0; i < mag_lut.size(); ++i) {
                    mag_lut[i] = float_to_bf16_rne(values[i]);
                }
                if (rtMemcpy((void *) workspace, qscale_lut.data(),
                             q2xs_nolut_lut_workspace::qscale_lut_bytes, rtMemcpyHostToDevice) != rtSuccess ||
                    rtMemcpy((void *) (workspace + q2xs_nolut_lut_workspace::qscale_lut_bytes), mag_lut.data(),
                             q2xs_nolut_lut_workspace::mag_lut_bytes, rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize Q2XS NoLUT LUT");
                }
            });
    return rpp_bind_managed_lut_workspace(ctx, managed_workspace, key.bytes);
}

static inline void q2xs_nolut_cdma_d2s_async(RPPdeviceptr sram_dst,
...
static inline void q2xs_nolut_cdma_s2d_async(RPPdeviceptr ddr_dst,
...
static inline void q2xs_nolut_copy_lut_workspace_to_sram(const q2xs_nolut_sram_io & io,
                                                         RPPdeviceptr               dev_lut_workspace,
                                                         RPPstream                  stream) {
    q2xs_nolut_cdma_d2s_async(io.sramB_qscale_lut, dev_lut_workspace, q2xs_nolut_lut_workspace::qscale_lut_bytes,
                              stream);
    q2xs_nolut_cdma_d2s_async(io.sramB_mag_lut, dev_lut_workspace + q2xs_nolut_lut_workspace::qscale_lut_bytes,
                              q2xs_nolut_lut_workspace::mag_lut_bytes, stream);
}

static inline void q2xs_nolut_cdma_copy_output_to_ddr(const q2xs_nolut_sram_io & io,
                                                      RPPdeviceptr               devC,
                                                      RPPstream                  stream) {
    q2xs_nolut_cdma_s2d_async(devC, io.sram_out, (size_t) io.sizeC, stream);
}
}  // namespace kernel_q2_xs_nolut
