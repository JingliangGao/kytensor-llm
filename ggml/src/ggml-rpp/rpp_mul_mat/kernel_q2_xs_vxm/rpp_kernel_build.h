...
static void q2xs_prepare_lookup_tables(rpp_kernel_context & ctx,
    ...
    key.variant = (uint32_t) GGML_TYPE_IQ2_XS;
    key.bytes   = workspace_bytes;
    ...
    RPPdeviceptr managed_workspace =
        rpp_dev_resource_manager::instance().get_or_create_lut_workspace(
            ctx.device, key, [=](RPPdeviceptr workspace, size_t) {
                ...
                if (rtMemcpy((void *) (workspace + off_qscale_lut), qscale_lut.data(), qscale_lut_bytes,
                             rtMemcpyHostToDevice) != rtSuccess ||
                    rtMemcpy((void *) (workspace + off_codebook_lut), codebook_lut.data(), codebook_lut_bytes,
                             rtMemcpyHostToDevice) != rtSuccess ||
                    rtMemcpy((void *) (workspace + off_sign_lut), sign_lut.data(), sign_lut_bytes,
                             rtMemcpyHostToDevice) != rtSuccess) {
                    throw std::runtime_error("failed to initialize Q2XS VXM LUT");
                }
            });
...
static void rpp_matmul_q2xs_vxm(rpp_kernel_context & ctx,
    ...
    rpp_module_load_once(ctx.rppBinMod, "rpp_kernel/matmul_q2xs_vxm.o");
    // -------------------------
