...
#include <vector>
    ...
    #if __has_include(<tongyi_decrypt_api.h>)
        #include <tongyi_decrypt_api.h>
        #define GGUF_TONGYI_DECRYPT_SUPPORTED 1
    #else
        #define GGUF_TONGYI_DECRYPT_SUPPORTED 0
    #endif
#else
    #define GGUF_TONGYI_DECRYPT_SUPPORTED 0
#endif

#define TONGYI_DECRYPT_SAM_PATH     "/usr/share/tongyi_decrypt_sam"
#define SAM_DEV_UUID               "0242ac110005"
...
struct gguf_context {
    ...
    // If model was decrypted before parsing, this points to the decrypted blob in memory.
    void * model_addr = nullptr;
    size_t model_size = 0;
};
...
static thread_local enum gguf_init_error g_gguf_last_init_error = GGUF_INIT_OK;

enum gguf_init_error gguf_get_last_init_error(void) {
    return g_gguf_last_init_error;
}

struct gguf_context * gguf_init_from_file(const char * fname, struct gguf_init_params params) {
    g_gguf_last_init_error = GGUF_INIT_OK;

    char    * decrypt_model_data = nullptr;
    uint64_t decrypt_model_size = 0;
    FILE    * file = nullptr;

    if (params.decrypt_type == GGUF_DECRYPT_TYPE_TONGYI) {
#if GGUF_TONGYI_DECRYPT_SUPPORTED
#if defined(_WIN32)
        GGML_LOG_ERROR("%s: tongyi decrypt is not supported on Windows in this build\n", __func__);
        return nullptr;
#else
        const int ret = tongyi_decrypt_model_with_abort(
            fname,
            TONGYI_DECRYPT_SAM_PATH,
            SAM_DEV_UUID,
            &decrypt_model_data,
            &decrypt_model_size,
            (tongyi_decrypt_abort_callback) params.decrypt_abort_callback,
            params.decrypt_abort_user_data);
        if (ret == TONGYI_DECRYPT_ERR_ABORTED) {
            g_gguf_last_init_error = GGUF_INIT_ERR_ABORTED;
            fprintf(stderr, "decrypt tongyi encrypted model %s aborted.\n", fname);
            return nullptr;
        }
        if (ret != 0) {
            g_gguf_last_init_error = GGUF_INIT_ERR_GENERAL;
            fprintf(stderr, "decrypt tongyi encrypted model %s failed ret=%d.\n", fname, ret);
            return nullptr;
        }

        file = fmemopen(decrypt_model_data, (size_t) decrypt_model_size, "rb");
        if (!file) {
            fprintf(stderr, "%s: failed to open '%s' that's in memory: '%s'\n", __func__, fname, strerror(errno));
            return nullptr;
        }
#endif
#else
        GGML_LOG_ERROR("%s: tongyi decrypt requested but tongyi_decrypt_api.h is not available\n", __func__);
        return nullptr;
#endif
    } else {
    ...
    if (result) {
        if (params.decrypt_type == GGUF_DECRYPT_TYPE_TONGYI) {
            result->model_addr = decrypt_model_data;
            result->model_size = (size_t) decrypt_model_size;
        }
    }
...
void * gguf_get_model_addr(const struct gguf_context * ctx) {
    return ctx ? ctx->model_addr : nullptr;
}

size_t gguf_get_model_size(const struct gguf_context * ctx) {
    return ctx ? ctx->model_size : 0;
}
