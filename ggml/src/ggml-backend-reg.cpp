...
struct ggml_backend_registry {
...
#endif
#ifdef GGML_USE_PONN
        register_backend(ggml_backend_ponn_reg());
#endif
#ifdef GGML_USE_BLAS
...
#endif
#ifdef GGML_USE_HOUMONPU
        register_backend(ggml_backend_houmonpu_reg());
#endif
#ifdef GGML_USE_CPU
    ...
    ggml_backend_reg_t load_backend(const fs::path & path, bool silent) {
        ...
        if (!handle) {
            if (!silent) {
		GGML_LOG_ERROR("%s: failed to load %s, err info: %s\n", __func__, path_str(path).c_str(), dlerror());
            }
            return nullptr;
        }
    ...
    // Remove dynamically loaded backends and their devices. This intentionally does NOT dlclose()
    // dynamic libraries for safety (see destructor comment).
    void reset_dynamic_backends(void) {
        // remove dynamic backends (leak handles to avoid dlclose)
        backends.erase(
            std::remove_if(backends.begin(), backends.end(), [](ggml_backend_reg_entry & entry) {
                if (!entry.handle) {
                    return false;
                }
                entry.handle.release(); // NOLINT
        ...
        // rebuild devices from remaining (static) backends
        devices.clear();
        for (const auto & entry : backends) {
            for (size_t i = 0; i < ggml_backend_reg_dev_count(entry.reg); i++) {
                devices.push_back(ggml_backend_reg_dev_get(entry.reg, i));
            }
        }
    }
};
...
void ggml_backend_unload_all(void) {
    get_reg().reset_dynamic_backends();
}
