...
extern "C" {
    ...
    enum gguf_decrypt_type {
        GGUF_DECRYPT_TYPE_NONE          = 0,
        GGUF_DECRYPT_TYPE_TONGYI        = 1,
        GGUF_DECRYPT_TYPE_SHANGTANG     = 2,
        GGUF_DECRYPT_TYPE_COUNT,
    };

    enum gguf_init_error {
        GGUF_INIT_OK            =  0,
        GGUF_INIT_ERR_GENERAL   = -1,
        GGUF_INIT_ERR_ABORTED   = -3,
    };

    // Called before each encrypted model file is decrypted.
    // Return 0 to continue, non-zero to abort (matches tongyi_decrypt_abort_callback).
    typedef int (*gguf_decrypt_abort_callback)(void * user_data);
    ...
    struct gguf_init_params {
        ...
        // if not NONE, optionally decrypt model blob before parsing metadata
        const enum gguf_decrypt_type decrypt_type;

        // optional: abort in-progress Tongyi decryption (may be NULL)
        gguf_decrypt_abort_callback decrypt_abort_callback;
    ...
    // After gguf_init_from_file returns NULL, inspect the failure reason.
    GGML_API enum gguf_init_error gguf_get_last_init_error(void);
    ...
    // If model was decrypted, these will point to the decrypted blob in memory.
    // They are only valid when decrypt_type != GGUF_DECRYPT_TYPE_NONE and decryption succeeded.
    GGML_API void *       gguf_get_model_addr(const struct gguf_context * ctx);
    GGML_API size_t       gguf_get_model_size(const struct gguf_context *ctx);
