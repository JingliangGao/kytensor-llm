...
static int32_t cpu_get_num_math_arm(int32_t physical_cores) {
    if (physical_cores <= 0) {
...
#if defined(__aarch64__) && defined(__linux__) && !defined(__ANDROID__)
    {
        int32_t physical = common_cpu_get_num_physical_cores();
...
void postprocess_cpu_params(common_cpu_params & cpuparams, const common_cpu_params * role_model, bool is_batch) {
    int32_t n_set = 0;
    ...
    if (cpuparams.n_threads <= 0) {
        ...
        if (role_model != nullptr) {
            ...
            if (is_batch && cpuparams.n_threads == common_cpu_get_num_math()) {
                cpuparams.n_threads = common_cpu_get_num_physical_cores();
            }
        } else {
...
common_init_result::common_init_result(common_params & params) :
    ...
    for (auto & la : params.lora_adapters) {
        ...
        lora.reset(llama_adapter_lora_init(model, la.path.c_str(), params.decrypt_type));
        if (lora == nullptr) {
...
common_init_result_ptr common_init_from_params(common_params & params) {
    ...
    postprocess_cpu_params(params.cpuparams_batch, &params.cpuparams, true);
...
struct llama_model_params common_model_params_to_llama(common_params & params) {
    ...
    mparams.decrypt_type            = params.decrypt_type;
    mparams.decrypt_abort_callback  = params.decrypt_abort_callback;
    mparams.decrypt_abort_user_data = params.decrypt_abort_user_data;
