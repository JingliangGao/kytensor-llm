...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"-dec", "--dec-type"}, "N",
        string_format("model decrypt type (default: %d)", params.decrypt_type),
        [](common_params & params, int value) {
            if (value < 0 || value >= (int) GGUF_DECRYPT_TYPE_COUNT) {
                throw std::invalid_argument("invalid --dec-type value");
            }
            params.decrypt_type = (enum gguf_decrypt_type) value;
        }
