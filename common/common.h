...
struct common_params_sampling {
    ...
    float   temp               = 0.00f;  // <= 0.0 to sample greedily, 0.0 to not output probabilities
...
struct common_params {
    ...
    // For GGUF encrypted models that require a decrypt pass before parsing/mapping.
    enum gguf_decrypt_type decrypt_type = GGUF_DECRYPT_TYPE_NONE;

    // optional decrypt abort (see llama_decrypt_abort_callback in llama.h)
    llama_decrypt_abort_callback decrypt_abort_callback = nullptr;
    void *                       decrypt_abort_user_data = nullptr;
    ...
    // server params
    int32_t port                = 17701;          // server listens on this network port
...
void postprocess_cpu_params(common_cpu_params & cpuparams, const common_cpu_params * role_model = nullptr, bool is_batch = false);
