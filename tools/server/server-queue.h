...
private:
	...
	bool req_enter_sleeping = false; // force unload even when idle sleep is disabled
...
public:
	...
	// force enter sleeping state (unload model via destroy()). blocks until sleeping
    // returns immediately if already sleeping. works even when idle_sleep_ms < 0
    void request_sleep();
