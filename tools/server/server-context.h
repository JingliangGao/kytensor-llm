...
struct server_context {
	...
	// single-process manual unload/load (reuses sleeping destroy()/load_model())
    // blocks until the operation completes on the task-loop thread
    // returns false with err_msg on failure
    bool request_unload(std::string & err_msg);
    bool request_load(const std::string & model_path, std::string & err_msg);

    // true when model is unloaded (sleeping)
    bool is_model_unloaded() const;

    // true when unloaded via POST /model/unload (inference must not auto-wake)
    bool is_manual_unloaded() const;
};
...
struct server_routes {
	...
	// single-process explicit model lifecycle (not available in router mode)
    server_http_context::handler_t post_model_load;
    server_http_context::handler_t post_model_unload;
