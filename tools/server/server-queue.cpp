...
void server_queue::request_sleep() {
    std::unique_lock<std::mutex> lock(mutex_tasks);
    if (sleeping) {
        QUE_DBG("%s", "already sleeping\n");
        return;
    }
    if (!running) {
        QUE_WRN("%s", "queue is not running, cannot enter sleep\n");
        return;
    }
    QUE_INF("%s", "requesting to enter sleeping state (manual unload)\n");
    req_enter_sleeping = true;
    condition_tasks.notify_one();
    condition_tasks.wait(lock, [&]{
...
void server_queue::start_loop(int64_t idle_sleep_ms) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            // no tasks, check for sleeping state (idle timeout or manual unload)
            if (should_sleep() || req_enter_sleeping) {
                QUE_INF("%s", "entering sleeping state\n");
                sleeping = true;
				req_enter_sleeping = false;
                ...
                bool res = condition_tasks.wait_for(lock, max_wait_time, [&]{
                    return (!queue_tasks.empty() || !running || req_enter_sleeping);
                });
                if (!running || !queue_tasks.empty()) {
                    break; // new task arrived or terminate
                }
                if (req_enter_sleeping) {
                    continue; // loop back to enter sleeping state
                }
                if (res) {
                    break;
                }
